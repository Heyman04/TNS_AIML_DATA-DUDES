...
The Streamlit application then displays the resulting customer persona.

---

# 📈 Cluster Visualization

The Streamlit dashboard generates a 2D scatter plot to visualize the customer clusters.
...
The visualization allows the user to observe the distribution of customers across the generated clusters and the position of a new customer relative to those clusters.
...
```bash
git checkout -b feature/aathika-ml
```

After completing the ML work:

```bash
git add .
git commit -m "Add customer dataset and KMeans model"
git push -u origin feature/aathika-ml
```

Create a Pull Request and merge it into `main` after review.

---

### Kamalini — Backend Branch

```bash
git checkout -b feature/kamalini-backend
```
...
git push -u origin feature/kamalini-backend
```
...
### Eesha — Frontend Branch

```bash
git checkout -b feature/eesha-frontend
```
...
git push -u origin feature/eesha-frontend
```
...
* Improve customer persona descriptions.
* Add interactive cluster filtering.
* Add customer history and segmentation reports.
* Deploy the FastAPI backend.
* Deploy the Streamlit frontend.
* Add Docker support.
* Add automated CI/CD using GitHub Actions.
...
The customer personas generated by the application are based on the available input features and the trained clustering model. They should not be treated as definitive classifications of real customers.
